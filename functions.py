...
def chat_with_gpt(file_name, data, clipboardContents):
    ...
    print(
        '"'
        + "\033[1m"
        + colored(jsonFile["promptName"], "yellow")
        + "\033[0m"
        + '" using model "'
        + selected_model
        + '". Prompt:'
    ...
    response = client.chat.completions.create(
        ...
        messages=[
            {"role": "system", "content": mergedSystemMessage},
...
def substitute_citations(output_text, citations):
    ...
    response = client.chat.completions.create(
        model=selected_model,
        messages=[
            {"role": "system", "content": message_when_fixing_citations},
            {"role": "user", "content": prompt_to_fix_citations},
        ],
        temperature=0.15,
    )
...
# Init
client = OpenAI(api_key=read_api_key(SETTINGS_FILENAME))
